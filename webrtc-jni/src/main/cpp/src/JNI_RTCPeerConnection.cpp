...
(JNIEnv * env, jobject caller, jobject jreceiver, jobject jcallback)
{
	...
	auto callback = rtc::make_ref_counted<jni::RTCStatsCollectorCallback>(env, jni::JavaGlobalRef<jobject>(env, jcallback));

	pc->GetStats(rtc::scoped_refptr<webrtc::RtpReceiverInterface>(receiver), callback);
}
...
(JNIEnv * env, jobject caller, jobject jsender, jobject jcallback)
{
	...
	pc->GetStats(rtc::scoped_refptr<webrtc::RtpSenderInterface>(sender), callback);
}
